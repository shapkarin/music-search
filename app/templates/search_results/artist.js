...
buf.push("<!--todo: preload images & auto_prefix delay-->");
var prettyNumber = function(x){ return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' '); }
buf.push("<div class=\"row byArtist\">");
// iterate results
;(function(){
  var $$obj = results;
...
buf.push("</div><div class=\"info\"><a" + (jade.attr("href", '' + (track.get("url")) + '', true, false)) + ">" + (jade.escape(null == (jade_interp = track.get('name')) ? "" : jade_interp)) + "</a><div class=\"stat\">Воспроизведено: " + (jade.escape((jade_interp =  prettyNumber(track.get('playcount'))) == null ? '' : jade_interp)) + "<br/>Слушателей: " + (jade.escape((jade_interp =  prettyNumber(track.get('listeners'))) == null ? '' : jade_interp)) + "</div></div></div></div>");
    }
